...
const madeForYou_service_1 = require("../services/madeForYou.service");
 ...
 */
router.post('/import', auth_middleware_1.isAuthenticated, auth_middleware_1.hasSpotifyConnected, async (req, res) => {
    try {
        const user = req.user;
        const body = req.body;
        const result = await madeForYou_service_1.madeForYouService.importFromSpotify(user.uid, body.skipIfExists ?? true);
        res.json(result);
    }
    catch (error) {
 ...
 */
router.get('/playlists', auth_middleware_1.isAuthenticated, async (req, res) => {
    try {
        const user = req.user;
        // Attempt to regenerate expired playlists silently
        try {
            await madeForYou_service_1.madeForYouService.regenerateExpired(user.uid);
        }
        catch (regenError) {
            console.warn('[MadeForYou] Background regeneration failed:', regenError);
            // Non-fatal — still return existing playlists
        }
        const playlists = await madeForYou_service_1.madeForYouService.getUserPlaylists(user.uid);
        res.json({ playlists });
    }
    catch (error) {
...
router.get('/playlists/:playlistId', auth_middleware_1.isAuthenticated, async (req, res) => {
    try {
        ...
        const playlist = await madeForYou_service_1.madeForYouService.getPlaylistById(user.uid, playlistId);
        if (!playlist) {
 ...
 */
router.post('/regenerate', auth_middleware_1.isAuthenticated, async (req, res) => {
    try {
        const user = req.user;
        const result = await madeForYou_service_1.madeForYouService.forceRegenerate(user.uid);
        res.json(result);
    }
    catch (error) {
 ...
 * }
 */
router.post('/events', auth_middleware_1.isAuthenticated, async (req, res) => {
    try {
        const user = req.user;
        const body = req.body;
        ...
        await madeForYou_service_1.madeForYouService.recordListeningEvent(user.uid, body);
 ...
 */
router.post('/events/batch', auth_middleware_1.isAuthenticated, async (req, res) => {
    try {
        const user = req.user;
        ...
        await madeForYou_service_1.madeForYouService.recordListeningEventsBatch(user.uid, events);
 ...
 */
router.get('/stats', auth_middleware_1.isAuthenticated, async (req, res) => {
    try {
        const user = req.user;
        const stats = await madeForYou_service_1.madeForYouService.getListeningStats(user.uid);
        res.json(stats);
    }
    catch (error) {
 ...
 */
router.delete('/data', auth_middleware_1.isAuthenticated, async (req, res) => {
    try {
        const user = req.user;
        await madeForYou_service_1.madeForYouService.deleteUserData(user.uid);
...
//# sourceMappingURL=madeForYou.routes.js.map