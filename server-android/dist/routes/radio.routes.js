...
router.get('/recommendations', async (req, res) => {
    try {
        const rawTrackId = req.query.trackId || '';
        ...
        const normalize = (str) => (str || '').toLowerCase().replace(/[\(\)\[\]"'\-_feat\.]/g, '').replace(/\s+/g, ' ').trim();
        if (cleanTitle)
            seenTitles.add(normalize(cleanTitle));
            ...
            try {
                ...
                if (Array.isArray(results) && results.length > 0) {
                    ...
                    for (const item of shuffled) {
                        ...
                        if (!itemName || seenTitles.has(normTitle))
                            continue;
                        if (item.id === rawTrackId || item.id === `yt-${rawTrackId}`)
                            continue;
                        ...
                        const itemArtists = typeof item.primaryArtists === 'string'
                            ? item.primaryArtists
                            : (Array.isArray(item.primaryArtists) ? item.primaryArtists.map((a) => a.name).join(', ') : (item.subtitle || artist));
