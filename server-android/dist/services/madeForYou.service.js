...
exports.madeForYouService = exports.MadeForYouService = void 0;
...
class MadeForYouService {
     ...
     */
    async importFromSpotify(userId, skipIfExists = true) {
        ...
        for (const sp of spotifyPlaylists) {
            ...
            const trackEntries = spotifyTracks.map((t, i) => ({
                trackId: t.id,
                position: i,
            ...
            const expiresAt = type === 'discover_weekly'
     ...
     */
    async createSeededPlaylists(userId) {
        const db = (0, firebase_1.getFirestore)();
        const now = new Date().toISOString();
        const results = [];
        ...
        const toEntries = (tracks, reason) => tracks.map((t, i) => ({
            trackId: t.id,
            position: i,
        ...
        const pickCover = (tracks) => tracks.find((t) => t.album.imageUrl)?.album.imageUrl;
     ...
     */
    async recordListeningEvent(userId, req) {
        const db = (0, firebase_1.getFirestore)();
        const now = new Date().toISOString();
        const event = {
            userId,
     ...
     */
    async recordListeningEventsBatch(userId, events) {
        const db = (0, firebase_1.getFirestore)();
        const batch = db.batch();
        const now = new Date().toISOString();
     ...
     */
    async regenerateExpired(userId) {
        const db = (0, firebase_1.getFirestore)();
        const now = new Date();
        ...
        for (const playlist of snapshot.docs.map((d) => d.data())) {
        ...
        // Aggregate and score
        const aggregated = (0, recommendation_engine_1.aggregateEvents)(events, allTracks);
        const artistAffinity = (0, recommendation_engine_1.buildArtistAffinity)(aggregated);
        const ranked = (0, recommendation_engine_1.computeScores)(aggregated, artistAffinity);
        ...
        for (const playlist of expiredPlaylists) {
            ...
            if (playlist.type === 'discover_weekly') {
                newTracks = (0, recommendation_engine_1.generateDiscoverWeekly)(ranked, allTracks, artistAffinity);
            }
            else if (playlist.type === 'daily_mix') {
                newTracks = (0, recommendation_engine_1.generateDailyMix)(ranked, playlist.mixNumber ?? 1);
            }
            ...
            if (newTracks.length === 0 && playlist.tracks.length > 0) {
                ...
                const expiresAt = playlist.type === 'discover_weekly'
                    ? new Date(Date.now() + DISCOVER_WEEKLY_INTERVAL_MS).toISOString()
                    : new Date(Date.now() + DAILY_MIX_INTERVAL_MS).toISOString();
...
exports.MadeForYouService = MadeForYouService;
exports.madeForYouService = new MadeForYouService();
//# sourceMappingURL=madeForYou.service.js.map