...
exports.buildArtistAffinity = buildArtistAffinity;
exports.generateDiscoverWeekly = generateDiscoverWeekly;
exports.generateDailyMix = generateDailyMix;
// ─── Tuning Constants ────────────────────────────────────────────────────────
...
function recencyDecay(lastPlayedAt, halfLifeDays) {
 ...
 */
function aggregateEvents(events, allTracks) {
 ...
 */
function computeScores(candidates, artistAffinity) {
    ...
    for (const entry of entries) {
        ...
        // Weighted sum
        let score = W_PLAY_COUNT * playSignal +
        ...
        let reason = 'frequently_played';
        if (completionSignal >= 0.85)
            reason = 'high_completion';
        if (affinitySignal >= 0.6)
            reason = 'similar_artist';
        if (recencySignal >= 0.8 && playSignal < 0.3)
            reason = 'recency_boost';
 ...
 */
function buildArtistAffinity(scores) {
 ...
 */
function generateDiscoverWeekly(rankedScores, allTracks, artistAffinity) {
    ...
    for (const ts of rankedScores) {
        if (result.length >= familiarCount)
            break;
        if (selected.has(ts.trackId))
            continue;
    ...
    for (const [, track] of allTracks) {
        if (selected.has(track.id))
            continue;
        const maxAffinity = Math.max(0, ...track.artists.map((a) => artistAffinity.get(a.name.toLowerCase()) ?? 0));
        if (maxAffinity > 0.2) {
    ...
    for (const ts of explorationCandidates) {
        if (result.length >= DISCOVER_WEEKLY_SIZE)
            break;
        if (selected.has(ts.trackId))
            continue;
        selected.add(ts.trackId);
 ...
 */
function generateDailyMix(rankedScores, mixNumber) {
    const result = [];
    const selected = new Set();
    ...
    for (const ts of pool) {
        if (result.length >= DAILY_MIX_SIZE)
            break;
        if (selected.has(ts.trackId))
            continue;
        // Only include tracks with reasonable completion
        if (ts.avgCompletion < 20 && ts.playCount > 2)
            continue;
        selected.add(ts.trackId);
        result.push(scoreToEntry(ts, result.length));
    }
...
// ─── Utilities ───────────────────────────────────────────────────────────────
function scoreToEntry(ts, position) {
    return {
        trackId: ts.trackId,
        position,
        score: ts.score,
