...
exports.youtubeMusicService = exports.YoutubeMusicService = void 0;
const axios_1 = __importDefault(require("axios"));
const crypto_js_1 = __importDefault(require("crypto-js"));
const firebase_1 = require("../config/firebase");
class YoutubeMusicService {
    constructor() {
        this.innerTubeUrl = 'https://music.youtube.com/youtubei/v1';
        this.apiKey = 'AIzaSyAO1spn4Vx86us6r2cK7vP7W50PgF059CE'; // Standard YouTube Music Web Client API Key
    }
    ...
    async refreshGoogleAccessToken(userId, refreshTokenDecrypted) {
        ...
        const response = await axios_1.default.post('https://oauth2.googleapis.com/token', params);
    ...
    async fetchHomeFeed(_userId, params) {
        try {
            const payload = {
            ...
            const response = await axios_1.default.post(`${this.innerTubeUrl}/browse?key=${this.apiKey}`, payload, {
                headers: {
            ...
            for (const section of sections) {
                ...
                if (Array.isArray(contents)) {
                    for (const itemNode of contents) {
                        ...
                        let subtitle = '';
                        if (r.subtitle?.runs) {
                            subtitle = r.subtitle.runs.map((run) => run.text).join('');
                        }
                        else if (r.flexColumns?.[1]?.musicResponsiveListItemFlexColumnRenderer?.text?.runs) {
                            subtitle = r.flexColumns[1].musicResponsiveListItemFlexColumnRenderer.text.runs.map((run) => run.text).join('');
                        }
                        // Navigation / IDs
                        let type = 'track';
                        let id = '';
    ...
    async fetchPlaylist(playlistId, overrideTitle) {
        try {
            const mappings = {
                // Rain Therapy shelf
            ...
            const response = await axios_1.default.post(`${this.innerTubeUrl}/browse?key=${this.apiKey}`, {
                browseId,
                context: {
                    client: {
                        clientName: 'WEB_REMIX',
            ...
            for (const r of rawTracks) {
                ...
                if (r.flexColumns?.[2]?.musicResponsiveListItemFlexColumnRenderer?.text?.runs?.[0]?.text) {
                    ...
                    const textLine = runs.map((x) => x.text).join('');
                    const parts = textLine.split(/\s*•\s*/).map((p) => p.trim());
                    if (parts.length > 1) {
                        albumName = parts[1];
                    }
                }
                ...
                if (r.fixedColumns) {
                    for (const col of r.fixedColumns) {
                        ...
                        if (text && /^\d{1,2}:\d{2}$|^\d{1,2}:\d{2}:\d{2}$/.test(text)) {
                            ...
                            if (t.length === 2)
                                durationMs = (t[0] * 60 + t[1]) * 1000;
                            else if (t.length === 3)
                                durationMs = (t[0] * 3600 + t[1] * 60 + t[2]) * 1000;
                            break;
                        }
                    }
                }
            ...
            if (tracks.length === 0) {
                ...
                for (const candidate of validCandidates) {
                    try {
                        ...
                        const candRes = await axios_1.default.post(`${this.innerTubeUrl}/browse?key=${this.apiKey}`, {
    ...
    async searchPlaylists(query) {
        try {
            const response = await axios_1.default.post(`${this.innerTubeUrl}/search?key=${this.apiKey}`, {
                query: query,
                    ...
                    if (shelf && shelf.contents && Array.isArray(shelf.contents)) {
                        for (const item of shelf.contents) {
                            ...
                            if (subtitleRuns && Array.isArray(subtitleRuns)) {
                                const textLine = subtitleRuns.map((x) => x.text).join('');
                                const parts = textLine.split(/\s*•\s*/).map((p) => p.trim());
                                if (parts.length > 0) {
...
exports.youtubeMusicService = new YoutubeMusicService();
//# sourceMappingURL=youtube-music.service.js.map